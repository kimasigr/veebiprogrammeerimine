<?php
	...
	function saveIdea($idea, $color){
		$notice = "";
		$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
		$stmt = $mysqli->prepare("INSERT INTO usersideas (userid, idea, color) VALUES(?, ?, ?)");
		echo $mysqli->error;
	...
	function listAllIdeas(){
		$ideasHTML = "";
		$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
		//$stmt = $mysqli->prepare("SELECT idea, color FROM usersideas");
		//$stmt = $mysqli->prepare("SELECT idea, color FROM usersideas WHERE userid = ?");
		$stmt = $mysqli->prepare("SELECT idea, color FROM usersideas WHERE userid = ? ORDER BY id DESC");
	...
	function latestIdea(){
		$mysqli = new  mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
		$stmt = $mysqli->prepare("SELECT idea FROM usersideas WHERE id = (SELECT MAX(id) FROM usersideas)");
