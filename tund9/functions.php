<?php
	...
	function signIn($email, $password){
		...
		$stmt = $mysqli->prepare("SELECT id, firstname, lastname, email, password FROM vp3users WHERE email = ?");
	...
	function saveIdea($idea, $color){
		$notice = "";
		$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
		$stmt = $mysqli->prepare("INSERT INTO vp3userideas (userid, idea, color) VALUES(?, ?, ?)");
		echo $mysqli->error;
	...
	function listAllIdeas(){
		$ideasHTML = "";
		$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
		//$stmt = $mysqli->prepare("SELECT idea, color FROM vp3userideas");
		//$stmt = $mysqli->prepare("SELECT idea, color FROM vp3userideas WHERE userid = ?");
		$stmt = $mysqli->prepare("SELECT id, idea, color FROM vp3userideas WHERE userid = ? AND deleted IS NULL ORDER BY id DESC");
...
//<p style="background-color: #ff2f2f">Kõik peavad rõõmust osa saama! | <a href="editusersideas.php?id=17">Toimeta</a></p>
	...
	function latestIdea(){
		$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
		$stmt = $mysqli->prepare("SELECT idea FROM vp3userideas WHERE id = (SELECT MAX(id) FROM vp3userideas WHERE deleted IS NULL)");
